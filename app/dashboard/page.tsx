...
export default async function DashboardPage() {
  ...
  const username = profile?.username ?? user.email ?? "Utente";

  const { data: contrada } = await supabase
    .from("contrade")
    .select("*")
    .eq("id", profile?.contrada_id)
    .single();

  const { data: classificaContrada } = await supabase
    .from("classifica_contrade")
    .select("*")
    .eq("contrada_id", profile?.contrada_id)
    .single();

  const { data: statistiche } = await supabase
    .from("statistiche_utenti")
    .select("*")
    .eq("user_id", user.id)
    .single();
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-3xl shadow-xl p-8 text-center hover:-translate-y-1 transition">
              ...
              <div className="my-6 flex justify-center">
                ...
                <Image
                  src={contrada?.logo ?? "/contrade/default.png"}
                  alt={contrada?.nome ?? "Contrada"}
                  width={90}
                  height={90}
                />
            ...
            <div className="grid md:grid-cols-3 gap-8">
              ...
              <div className="bg-amber-50 rounded-2xl p-6 text-center">
                ...
                <p className="text-5xl font-extrabold text-amber-700 mt-3">
                  {statistiche?.punti ?? profile?.punti ?? 0}
                </p>
              ...
              <div className="bg-amber-50 rounded-2xl p-6 text-center">
                ...
                <p className="text-5xl font-extrabold mt-3">
                  {statistiche?.posizione_generale ?? "-"}°
                </p>
              ...
              <div className="bg-amber-50 rounded-2xl p-6 text-center">
                ...
                <p className="text-5xl font-extrabold text-green-600 mt-3">
                  {statistiche?.posizione_contrada ?? "-"}°
                </p>
